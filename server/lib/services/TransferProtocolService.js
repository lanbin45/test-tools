 ...
 *
 * This file is used to implement logic of TransferProtocol request.
 */
...
const log = require('../utils/PGLog').getLogger('TransferService');
...
const service = function(requestParams, callback) {
    ...
    Async.waterfall([
        ...
        function(result, innerCB) {
            ...
            let protocolExists = IUtils.hasProtocols(notifiedCode, 3);
            if (!protocolExists) {
        ...
        function(innerCB) {
            ...
            httpClient.sendGetRequest(Constants.API_GETTRANSFERPROTOCOL, param, (err, resultArray) => {
                if (!!err || !resultArray || resultArray.length <= 0) {
                    let msg = 'Fail to send GetTransferProtocol request to PG.';
                    log.error(msg);
                    innerCB(msg);
                    return;
                }
                ...
                for (let i = 0; i <resultArray.length; ++i) {
                    ...
                    if (resultArray[i].statusCode != 200) {
                        let msg = 'One GetTransferProtocol failed.';
                        log.error(msg);
                        innerCB(msg);
                        return;
        ...
        function(zipArray, innerCB) {
            ...
            httpClient.sendPostRequest(Constants.API_CLEARTRANSFERQUEUE, param, (err, result) => {
                if (!!err || !result || result.statusCode != 200) {
                    let msg = 'Fail to send ClearTransferQueue request to PG.';
                    log.error(msg);
                    innerCB(msg);
                    return;
                }
